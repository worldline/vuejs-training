<template>
<div id="login-form">
  <form @submit.prevent="loggedIn = true" v-if="loggedIn === false">
    <h1>{{ title }}</h1>
  ...
  <ul class="films" v-else>
    <li class="film card" v-for="film in films" :key="film.title+film.released">
        <img class="poster" :src="film.poster" />
        <p class="title">
            {{ film.title }}
            <span class="rating" v-if="film.metascore !== 'N/A'">
            {{ '★'.repeat(Math.ceil(film.metascore/20)) }}
            </span>
        </p>
        <dl>
            <dt>Release date</dt>
